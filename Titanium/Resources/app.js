...
function createLevel() {
	...
	lblScore = Titanium.UI.createLabel({
		...
		font: {
			fontSize: 16,
			fontWeight: 'bold',
			fontFamily: 'Courier, Arial, Verdana'
		}
	});
	win1.add(lblScore);
	...
	if (isPlaying) {
		...
		if (ball.top + ball.height > 480) {
			...
			alertDialog.addEventListener('click', function(ev) {
				Titanium.API.info("Restart " + ev.cancel);
				
				// there use to be an exit button, but Apple would be sad 
				// if you actually did that.
				
				switch (ev.index) {
				case 0:
					Titanium.API.info("Restart button was hit");
					win1.remove(paddle);
					win1.open();
					createLevel();
					playGame();
